...
(function() {
    ...
    document.addEventListener("DOMContentLoaded", function(event) {
        ...
        function make_drawer(name, slider_count, args) {
            ...
            if (!el) {
                // Skip this drawer if container is missing
                return ret;
            }

            let drawer = new Drawer(el, name);
            ret.push(drawer);
            ...
            for (let i = 0; i < slider_count; i++) {
                let sliderEl = document.getElementById(name + "_sl" + i);
                if (!sliderEl) continue; // skip missing sliders

                let slider = new Slider(sliderEl, function(x) {
                    if (i == 0) drawer.set_arg0(x);
                    else if (i == 1) drawer.set_arg1(x);
                    else if (i == 2) drawer.set_arg2(x);
        ...
        // signal8 segmented control
        let sig8El = document.getElementById("signal8_seg0");
        if (sig8El) {
            signal8_seg = new SegmentedControl(sig8El, function(x) {
        ...
        // gps_orbits1 segmented control
        let seg1El = document.getElementById("gps_orbits1_seg0");
        if (seg1El && gps_orbits1.length) {
            new SegmentedControl(seg1El, function(x) {
        ...
        if ("IntersectionObserver" in window) {
            ...
            all_containers.forEach(container => {
                if (container.drawer) container.drawer.set_visible(true);
            });
        }
